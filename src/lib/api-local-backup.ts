import { openSourceLLM, chat } from './open-source-llm';
...
// Table IDs (for local DB organization)
const TABLES = {
  MVP_PROJECTS: 'mvp_projects',
  ROADMAP_DOCUMENTS: 'roadmap_documents',
...
`;
      ...
      try {
        response = await ai.createChatCompletion({
          model: 'default',  // Replace with available model
          messages: [
            {
              role: 'system',
